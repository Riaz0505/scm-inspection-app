...
export const GarmentModel: React.FC<GarmentModelProps> = ({ 
  ...
  // Decide if we should show the toggle (only if we have back image or layout image + default behavior or custom points from both sides)
  const hasMultipleSides = customPoints?.some(p => p.id.startsWith('F-')) && customPoints?.some(p => p.id.startsWith('B-'));
  const showToggle = (frontImageUrl && backImageUrl) || (!customPoints && !layoutImage) || hasMultipleSides;
